...
def drawPred(frame, classId, conf, left, top, right, bottom,myFlag):
    ...
    cv2.rectangle(frame, (left, top), (right, bottom), (0, 300, 0), 3)
    print(top, left, bottom, right)
    # Display the resulting frame 
    cv2.imshow('Frame', frame.astype(np.uint8))
    name = 'frame' + str(currentFrame) + '.jpg'
    # frame[y:y+h, x:x+w]
    cv2.imwrite('D:\\ANPR\\AutomaticParkingSystemANPR\\Temporary frames' + name, frame[top:bottom, left:right])
    image = cv2.imread("D:\\ANPR\\AutomaticParkingSystemANPR\\Temporary frames" + name)
...
def forDesign(button):
    ...
    # path = 0
    path = "D:\\ANPR\\AutomaticParkingSystemANPR\\ferrari.mp4"
    main(path, button)
...
if __name__=="__main__":
    while True:
        ...
        # path = "D:\\ANPR\\AutomaticParkingSystemANPR\\workingVid.mp4"
        # path = 0
        path = "D:\\ANPR\\AutomaticParkingSystemANPR\\ferrari.mp4"
        button = int(input())
